...
class Fileuploads { 
    ...
    public function image($files, $file_header='', $folder='') { // {{{
        ...
        if(!empty($files)) {
            ...
            if(!empty($file_name) && !empty($tmp_name)) {
                ...
                $targetFilename = ((!empty($folder))?$folder.'/':'').$file_header.'_'.$code;
                ...
                $finalFullname = PAGEHOST.'/'.$this->s3_upload_path."/".$targetFilename.".".$fileParts['extension'];
                //$targetThumbname = "/uploads/".$targetFilename."_thumb.".$fileParts['extension'];
                $fileTypes = array('jpg','jpeg','png', 'ico', 'gif'); // File extensions
                if(in_array($fileParts['extension'],$fileTypes)) {
                    // 폴더가 있는지 체크 - 없으면 만들기
                    $new_path = rtrim($targetPath,'/').'/'.((!empty($folder))?$folder:'');
